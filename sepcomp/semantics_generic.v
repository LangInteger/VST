...
Record CoreSemantics {C M V : Type} : Type :=
  { initial_core : nat -> M -> C -> M -> V -> list V -> Prop
  ; at_external : C -> M -> option (external_function * list V)
  ; after_external : option V -> C -> M -> option C
  ; halted : C -> int -> Prop
